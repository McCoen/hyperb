...
__kernel void gradientAt(double energyInt, __global double* grad, __global double* wt, __global double* st, __global double* damperX, __global double* wtBounds, __global double* pu, double a, double l, double t, __global int* noFault) {
	...
	for (int i = 0; i < NUMBER_OF_DAMPERS; i++) {
		dampx[i] = damperX[i];
		dbounds[i * 2] = wtBounds[i * 2];
		dbounds[1 + i * 2] = wtBounds[1 + i * 2];
		for (int j = 0; j < K + 1; j++) {
